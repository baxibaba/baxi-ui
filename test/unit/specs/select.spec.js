...
describe('Select', () => {
  ...
  it('select default', done => {
    ...
    const selectPicker = vm.$el.querySelector('.baxi-drawer-container')
    expect(selectPicker.style.display).to.equal('none')

    vm.$el.querySelector('input').click()
    setTimeout(() => {
      expect(vm.$el.querySelector('.baxi-drawer-container').style.display).to.equal('')
