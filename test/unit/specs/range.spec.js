...
describe('Range', () => {
  ...
  it('default range', () => {
    ...
    const buttonWrap = vm.$el.querySelector('.baxi-range-button-wrap')
  ...
  it('disabled range', () => {
    ...
    let rangeElm = vm.$el
    expect(rangeElm.classList.contains('baxi-range-disabled')).to.be.true
  })
  /**
   * 设置区间
   */
  it('show-stops range', () => {
    ...
    const len = vm.$el.querySelectorAll('.baxi-range-stop').length
  ...
  it('range event touch range', done => {
    ...
    const buttonWrap = vm.$el.querySelector('.baxi-range-button-wrap')
    const range = vm.$el.querySelector('.baxi-range-wrap')
    setTimeout(() => {
      dispatchSwipe(range, [
        {
