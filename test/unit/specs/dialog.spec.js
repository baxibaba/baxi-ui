...
describe('Dialog', () => {
  ...
  it('should render correct contents - not btns', () => {
    ...
    const btns = vm.$el.querySelector('.baxi-dialog-btns').getElementsByTagName('div')
  ...
  it('btns callback events', () => {
    ...
    vm.show()
    const cancel = vm.$el.querySelector('.baxi-dialog-btns div:first-child')
    cancel.click()
    expect(cancelHandler).to.be.calledOnce

    vm.show()
    const confirm = vm.$el.querySelector('.baxi-dialog-btns div:last-child')
    confirm.click()
  ...
  it('dialog api', () => {
    ...
    const cancel = vm.ins.$el.querySelector('.baxi-dialog-btns  div:first-child')
    cancel.click()
    expect(cancelHandler).to.be.calledOnce

    vm.show()
    const confirm = vm.ins.$el.querySelector('.baxi-dialog-btns  div:last-child')
    confirm.click()
    expect(confirmHandler).to.be.calledOnce
  })
})