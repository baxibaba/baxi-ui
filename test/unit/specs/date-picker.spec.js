...
describe('DatePicker', () => {
  ...
  it('date-picker default data', () => {
    ...
    const scroll = vm.$el.querySelectorAll('.baxi-picker-container-scroll')
    expect(scroll.length).to.equal(3)

    vm.show()
    const cancel = vm.$el.querySelector('.baxi-picker-cancel')
    cancel.click()
    expect(cancelHandler).to.be.calledOnce

    vm.show()
    const confirm = vm.$el.querySelector('.baxi-picker-confirm')
    confirm.click()
  ...
  it('date-picker setData type string', done => {
    ...
    setTimeout(() => {
      scroll = vm.$el.querySelectorAll('.baxi-picker-container-scroll')
  ...
  it('date-picker min year scroll', done => {
    vm = createVue({
      template: `
        <baxi-date-picker ref="datePicker"
    ...
    vm.visible = true
    let scroll = vm.$el.querySelectorAll('.baxi-picker-container-scroll')
