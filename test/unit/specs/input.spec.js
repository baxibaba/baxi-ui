...
describe('Input', () => {
  ...
  it('disabled input', () => {
    ...
    let buttonElm = vm.$el
    expect(buttonElm.classList.contains('baxi-input')).to.be.true
  ...
  it('value should be empty when clear button clicked', done => {
    ...
    vm.$el.querySelector('.baxi-input-clear').click()
    setTimeout(() => {
  ...
  it('type status  should switch when you click on eye', done => {
    ...
    vm.$el.querySelector('.baxi-input-eye').click()
  ...
  it('handle events', () => {
    ...
    const baxiInput = vm.$refs.baxiInput
    baxiInput.handleFocus()
    expect(focusHandler).to.be.called

    baxiInput.handleBlur()
    expect(blurHandler).to.be.called

    baxiInput.handleChange()
