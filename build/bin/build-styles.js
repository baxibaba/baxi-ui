...
function taskbaxiUi () {
  return gulp.src(resolve('src/styles/index.less'))
    ...
    .pipe(cleanCSS())
    .pipe(rename('baxi-ui.css'))
    .pipe(gulp.dest(resolve('lib/styles')))
}

function taskbaxiUiPx () {
  return gulp.src(resolve('src/styles/index.less'))
    .pipe(less())
    .pipe(autoprefixer(ap))
    .pipe(cleanCSS())
    .pipe(rename('baxi-ui-px.css'))
    .pipe(gulp.dest(resolve('lib/styles')))
}
...
gulp.task('default', gulp.parallel(taskbaxiUi, taskbaxiUiPx, iconfontTask, taskPackages, taskPackagesPx))
